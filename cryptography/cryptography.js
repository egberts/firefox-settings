...
// NOTICE: OCSP leaks your IP and domains you visit to the CA when OCSP
// Stapling is not available on visited host
// NOTICE: OCSP is vulnerable to replay attacks when nonce is not configured
// on the OCSP responder
// NOTICE: OCSP adds latency (performance)
// NOTICE: Short-lived certificates are not checked for revocation
// (security.pki.cert_short_lifetime_in_days, default:10)
...
// NOTICE: Firefox falls back on plain OCSP when must-staple is not
configured on the host certificate
...
// Disabling this will make OCSP bypassable by MitM attacks suppressing OCSP
// responses
// NOTICE: `security.OCSP.require` will make the connection fail when the
// OCSP responder is unavailable
// NOTICE: `security.OCSP.require` is known to break browsing on some
...
// 1 = TLS 1.0 is the minimum required / maximum supported encryption
// protocol. (This is the current default for the maximum supported version.)
// 2 = TLS 1.1 is the minimum required / maximum supported encryption
// protocol.
...
// PREF: Warn the user when server doesn't support RFC 5746 ("safe"
// renegotiation)
//
...
// https://web.nvd.nist.gov/view/vuln/detail?vulnId=CVE-2009-3555
// TODO: `security.ssl.require_safe_negotiation` is more secure but makes
// browsing next to impossible (2012-2014-... - `ssl_error_unsafe_negotiation`
...
// we could also disable security.ssl.errorReporting.enabled, but I think
// it's
...
// PREF: Pre-populate the current URL but do not pre-fetch the certificate in
// the "Add Security Exception" dialog
